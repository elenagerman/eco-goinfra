...
func TestLocalVolumeSetDelete(t *testing.T) {
	testCases := []struct {
		...
		{
			testLocalVolumeSet: buildValidLocalVolumeSetObjectBuilder(clients.GetTestClients(clients.TestClientParams{})),
			expectedError:      fmt.Errorf("localVolumeSet cannot be deleted because it does not exist"),
		},
	}
...
func TestLocalVolumeSetWithGeneration(t *testing.T) {
	testCases := []struct {
		testGeneration    int64
		expectedErrorText string
	}{
		{
			testGeneration:    int64(42),
			expectedErrorText: "",
		},
		{
			testGeneration:    int64(0),
			expectedErrorText: "'generation' argument cannot be equal zero",
		},
	}
	...
	for _, testCase := range testCases {
		...
		result := testBuilder.WithGeneration(testCase.testGeneration)
		assert.Equal(t, testCase.expectedErrorText, result.errorMsg)

		if testCase.expectedErrorText == "" {
			assert.NotNil(t, result)
			assert.Equal(t, testCase.testGeneration, result.Definition.Generation)
		}
	}
}
